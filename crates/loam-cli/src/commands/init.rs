...
use std::{ 
    io,
    fs::{copy, create_dir_all, remove_dir_all, metadata, read_dir, Metadata,},
path::Path};
...
impl Cmd {
    ...
    pub fn run(&self) -> Result<(), Error> {
        ...
        remove_dir_all(Path::new(&self.project_path.clone()).join("contracts/hello_world/")).map_err(|e| {
        ...
        copy_example_contracts(Path::new(LOAM_EXAMPLES), Path::new(&self.project_path.clone()), &examples)?;
